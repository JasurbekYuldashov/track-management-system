...
import { Card, Col, CardBody, Nav, NavItem, NavLink, Button } from "reactstrap";
...
class Trips extends React.Component {
                        ...
                        <div className="notes-col">
                            {!params.data.notes ? (
                                <Button.Ripple
                                    ...
                                    type="button"
                                    onClick={() =>
                                        this.handleToggle(params.data.truckId)
                                    }
                                >
                                    Add note
                                </Button.Ripple>
    ...
    getLastSaturday(theDate) {
        ...
        var lastSaturday = new Date(
            new Date().setDate(
                dateToUse.getDate() -
                    (dateToUse.getDay() === 0 ? 7 : dateToUse.getDay() + 1)
            )
        );
        return lastSaturday;
    }
    ...
    getLatestSaturday(theDate) {
        var dateToUse = new Date(theDate);
        var latestSaturday = new Date(
            new Date().setDate(dateToUse.getDate() - dateToUse.getDay() + 1)
        );
        ...
        if (this.state.page !== prevState.page) {
            ...
            fetch(
                process.env.REACT_APP_BASE_URL +
                    `/dashboard?sort=id,DESC&size=10&page=${this.state.page}${
    ...
    handleMenuClick = (event, truckId, unitStatusId) => {
        ...
        const currentUnitStatuses = this.state.unitStatuses?.filter(
            (x) => x.id !== unitStatusId
        );
        this.setState({
    ...
    changeUnitStatus = (status) => {
        fetch(
            process.env.REACT_APP_BASE_URL +
                `/unit/update_status/${this.state.statusChangeId}/${status.id}`,
            {
                headers: {
                    Authorization: this.props.token,
    ...
    addNote = () => {
        fetch(process.env.REACT_APP_BASE_URL + "/truck_notes", {
            headers: {
                Authorization: this.props.token,
                "Content-Type": "application/json",
            },
    ...
    getNotes = (truckId) => {
        fetch(
            process.env.REACT_APP_BASE_URL +
                `/truck_notes?truck_id=${truckId}&page=0&size=1000000`,
            {
                headers: {
                    Authorization: this.props.token,
                    "Content-Type": "application/json",
    ...
    componentDidMount() {
        fetch(
            process.env.REACT_APP_BASE_URL +
                `/owned_company/all_for_dashboard?sort=id,DESC&size=10`,
            {
                headers: {
                    Authorization: this.props.token,
                    "Content-Type": "application/json",
        ...
        fetch(process.env.REACT_APP_BASE_URL + "/dashboard/context", {
            headers: {
                Authorization: this.props.token,
                "Content-Type": "application/json",
            },
            ...
            <>
                ...
                >
                    {this.state.currentUnitStatuses?.map((status) => {
                        return (
                            <MenuItem
                                onClick={() => this.changeUnitStatus(status)}
                            >
                                {status.name}
                            </MenuItem>
                        );
