...
class Drivers extends React.Component {
    ...
    updateInfo = (pageNumber) => {
        ...
        fetch(
            process.env.REACT_APP_BASE_URL +
                `/fleet/list?${number && `&number=${number}`}${
                    vin && `&vin=${vin}`
                }`,
            {
                headers: {
            ...
            .then((data) => {
                ...
                data.data.forEach((el, i) => {
                    ...
                    // });
                    let date = null;
                    if (el.licenseExpiration)
                        date = new Date(el.licenseExpiration);
    ...
    updateAllNotifications = () => {
        fetch(
            process.env.REACT_APP_BASE_URL +
                `/fleet/expiration_notifications?size=10`,
            {
                headers: {
                    Authorization: this.props.token,
                },
    ...
    updateNotifications = () => {
        fetch(
            process.env.REACT_APP_BASE_URL +
                `/fleet/expiration_notifications?was_seen=false&size=10`,
            {
                headers: {
                    Authorization: this.props.token,
                },
    ...
    readNotification = (id) => {
        ...
        fetch(
            process.env.REACT_APP_BASE_URL +
                `/fleet/expiration_notification/${id}`,
            {
                headers: {
                    Authorization: this.props.token,
                },
                ...
                <Card className="agGrid-card">
                    <div className="d-flex justify-content-between align-items-center mt-2 mx-2 mb-1">
                        ...
                        <div className="d-flex align-items-center">
                            <h4 className="mx-2 mb-0 text-nowrap">
                                Global search
                            </h4>
                            <Input
                                type="text"
