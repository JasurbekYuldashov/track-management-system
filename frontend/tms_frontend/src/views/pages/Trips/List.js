...
class Trips extends React.Component {
    ...
    componentDidMount() {
        ...
        fetch(
            process.env.REACT_APP_BASE_URL + "/trip/list?sort=id,DESC&size=40",
            {
                headers: {
                    Authorization: this.props.token,
                },
    ...
    toggle = (tab) => {
        if (this.state.active !== tab) {
            ...
            if (tab == 0) {
                ...
                fetch(
                    process.env.REACT_APP_BASE_URL +
                        "/trip/list?sort=id,DESC&size=40&page=0",
                    {
                        headers: {
                            Authorization: this.props.token,
                        },
                ...
                fetch(
                    process.env.REACT_APP_BASE_URL +
                        `/trip/list?status_id=${tab}&sort=id,DESC&size=40&page=0`,
                    {
                        headers: {
                            Authorization: this.props.token,
                        },
    ...
    deleteTrip = () => {
        fetch(
            process.env.REACT_APP_BASE_URL + `/trip/${this.state.deletingId}`,
            {
                headers: {
                    Authorization: this.props.token,
                },
