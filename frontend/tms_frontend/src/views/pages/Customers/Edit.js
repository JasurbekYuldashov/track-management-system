...
class NewCompany extends React.Component {
  ...
  updateOffices = () => {
    fetch(`/company/offices/${this.props.match.params.id}`, {
      headers: {
        Authorization: this.props.token,
      },
    })
  ...
  componentDidMount() {
    fetch("/company/context", {
      ...
      .then((data) => {
        ...
        fetch(`/company/${this.props.match.params.id}`, {
          headers: {
            Authorization: this.props.token,
          },
        })
          ...
          .then((data) => {
            ...
            if (data.locationId) {
              fetch(`/location/by_id/${data.locationId}`)
                .then((res) => res.json())
                .then((data) => {
                  this.setState({
                    ...
                    <FormGroup className="align-items-center" row>
                      ...
                      <Col md="8">
                        <AsyncSelect
                          ...
                          placeholder="Search"
                          isClearable={true}
                          value={this.state.defaultStateProvince}
                          loadOptions={this.loadOptions}
                          onChange={(value) => {
                            if (value !== null) {
                              this.setState({
                                stateProvince: value.value,
                                defaultStateProvince: value,
                              });
                            } else
                              this.setState({
                                stateProvince: null,
                  ...
                  return (
                    <React.Fragment key={item.id}>
                      ...
                      <Button.Ripple
                        ...
                        color="red"
                        type="button"
                        onClick={() => this.deleteOffice(item.id)}
                      >
                        <Icon.Trash2 />
                      </Button.Ripple>
                    </React.Fragment>
