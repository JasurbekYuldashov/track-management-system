...
class EditLoads extends React.Component {
  ...
  newLoad = () => {
    ...
    fetch("/load/edit", {
      ...
      {
        if (res.ok) {
          toast.success("Load successfuly edited", { transition: Flip });
          window.history.back();
        } else {
          let result = res.json();
          try {
  ...
  postPayments = () => {
    ...
    fetch(`/counting/on_load`, {
      ...
      if (res.ok) {
        toast.success("Payments successfuly edited", { transition: Flip });
      } else {
        let result = res.json();
        try {
          result.then((data) => {
      ...
      .then((data) => {
        ...
        fetch("/load/context", {
          ...
          .then((data) => {
            ...
            fetch(`/load/${this.props.match.params.id}`, {
              headers: {
                Authorization: this.props.token,
              },
            })
              ...
              .then((data) => {
                ...
                this.setState({
                  ...
                  canBeChanged: data.canBeChanged || this.props.userRole === 'admin',
                  canBeChangedInitial: data.canBeChanged,
                  prevRateConfirmationId: data.rateConfirmationId,
                  prevRevisedRateConfirmationId: data.revisedRateConfirmationId,
                  loading: false,
                });
                ...
                fetch(`/counting/on_load/${this.props.match.params.id}`, {
                  headers: {
                    Authorization: this.props.token,
                  },
                })
                  ...
                  .then((data) => {
                    ...
                    document.querySelector("#detention").value = data.detention;
                    document.querySelector("#dispute").value = data.dispute;
                    document.querySelector("#factoring").value = data.factoring;
  ...
  setAbbreviation = (e) => {
    let abbreviation = this.state.owned_companies.find((item) => item.id == e);
    this.setState({
  ...
  render() {
    return (
      <>
        <Card>
          <CardHeader>
            <h3 className="mb-0">Editing Load {this.state.canBeChangedInitial ? <AiOutlineUnlock /> : <AiOutlineLock />}</h3>
          </CardHeader>
          <CardBody>
            {this.state.loading ? (
              <Spin
                ...
                <FormGroup className="align-items-center mt-2" row>
                  ...
                  <Col md="8">
                    <CustomInput
                      ...
                      name="select"
                      id="owned_company"
                      onChange={(e) => this.setAbbreviation(e.target.value)}
                    >
                  ...
                  {this.state.prevRateConfirmationId ? (
                    ...
                    <Col md="2">
                      <CustomInput
                        type="file"
                        onInput={(e) =>
                          this.uploadFile(e.target.files[0], "rc")
                        }
                      />
                    </Col>
                  )}
                  ...
                  {this.state.prevRevisedRateConfirmationId ? (
                    ...
                    <Col md="2">
                      <CustomInput
                        type="file"
                        onInput={(e) =>
                          this.uploadFile(e.target.files[0], "rrc")
                        }
                      />
                    </Col>
                  )}
            ...
            <Card>
              ...
              <CardBody>
                ...
                <Button
                  ...
                  className="d-flex align-items-center"
                  type="button"
                  onClick={() => this.toggleDeliveryModal(null)}
                >
                  <Icon.Plus size={22} />
                  Add new delivery
                </Button>
