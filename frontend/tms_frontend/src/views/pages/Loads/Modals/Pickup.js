...
class PickupModal extends React.Component {
  ...
  newPickup = () => {
    let sendingData = {
      ...
      bolId: this.state.prevBolId ? this.state.prevBolId : this.state.bolId,
    };
      ...
      fetch("/pickup/new", {
        ...
        .then((data) => {
          fetch(`/pickup/resolved_date/${sendingData.pickupDate_}`)
            .then((res) => res.text())
            .then((time) => {
              toast.success("Pickup successfuly added", { transition: Flip });
              this.props.addPickup(
                this.state.shipperName,
                time,
                data.id,
            ...
            <FormGroup className="align-items-center" row>
              ...
              {this.state.prevBolId ? (
                ...
                <Col md="8">
                  <CustomInput
                    type="file"
                    onInput={(e) => this.uploadBol(e.target.files[0])}
                  />
                </Col>
              )}
            </FormGroup>
