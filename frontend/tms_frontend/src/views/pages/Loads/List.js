...
class List extends React.Component {
    ...
    deleteLoad = () => {
        fetch(
            process.env.REACT_APP_BASE_URL + `/load/${this.state.deletingId}`,
            {
                headers: {
            ...
            fetch(
                process.env.REACT_APP_BASE_URL +
                    "/load/list?sort=id,DESC&size=40&tripId=-1",
                {
                    headers: {
                        Authorization: this.props.token,
                    },
    ...
    componentDidUpdate(prevProps, prevState) {
        if (
            ...
            fetch(
                process.env.REACT_APP_BASE_URL +
                    `/load/list?sort=id,DESC&size=40&tripId=-1&page=${
                        this.state.page
    ...
    componentDidMount() {
        ...
        fetch(
            process.env.REACT_APP_BASE_URL +
                "/load/list?sort=id,DESC&size=10&tripId=-1",
            {
                headers: {
                    Authorization: this.props.token,
                },
            ...
            .then((data) => {
                ...
                data.content.forEach((el, i) => {
                    let elToShow = {
                        ...
                        trip: el.tripId,
                        status:
                            el.tripId == null ? "NOT DISPATCHED" : "DISPATCHED",
                        pickup: el.pickupDateFormatted,
                        delivery: el.deliveryDateFormatted,
                        customer: el.customer,
                        from: el.from,
